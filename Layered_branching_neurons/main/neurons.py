...
class Neuron(object):
    def __init__(self, i, j, id, layer):
        self.i = i
        self.j = j
        self.x = j * SPACING
        self.y = i * SPACING
    ...
    def build_neighs(self, all_neurons):
        while len(self.neigh_list) < N_NEIGH:
            ...
            check_layer = (self.layer == new_n.layer - 1)
            check_prev = (not new_n.id in self.neigh_list)
            if check_layer and check_prev:
    ...
    def spont_fire(self):
        # Check for spontaneous firing if first layer
        if self.layer == 0:
            self.next_state += 1
